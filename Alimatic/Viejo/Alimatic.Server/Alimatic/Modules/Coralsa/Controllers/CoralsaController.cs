...
namespace Alimatic.Coralsa.Controllers
{
    ...
    [Controller(Route = nameof(Coralsa))]
    class CoralsaController : BaseController
    {
        ...
        //public async Task<CoralsaData> Data() => new CoralsaData
        //{
        //    Rows = await Rows(0),
        ...
        public async Task<InitialDataApiModel> Data()
        {
            var xx = new InitialDataApiModel();
        ...
        public async Task<IEnumerable<ModelApiModel>> Models()
        {
            ...
            if ((userApiModel?.SecurityLevel ?? -1) == -1)
                return (await CoralsaDbContext.Models.ToListAsync()).Select(p => Node.Mapper.Map<ModelApiModel>(p));

            var models = from model in CoralsaDbContext.Models
                         join userModel in CoralsaDbContext.UserModels
        ...
        public async Task<IEnumerable<FrequencyApiModel>> Frequencies()
            => (await CoralsaDbContext.Frequencies.ToListAsync()).Select(p => Node.Mapper.Map<FrequencyApiModel>(p));

        public async Task<IEnumerable<DivisionApiModel>> Divisions()
            => (await CoralsaDbContext.Divisions.ToListAsync()).Select(p =>
        ...
        public async Task<IEnumerable<RowApiModel>> Rows(int modelo)
            => (await CoralsaDbContext.Rows.Where(p => modelo != 0 ? modelo == p.ModelId : true)
                .ToListAsync()).Select(p => Node.Mapper.Map<RowApiModel>(p));

        public async Task<IEnumerable<GroupApiModel>> Groups(int division)
            => (await CoralsaDbContext.Groups.Where(p => division != 0 ? division == p.DivisionId : true)
                .ToListAsync()).Select(p => Node.Mapper.Map<GroupApiModel>(p));

        public async Task<IEnumerable<EnterpriseApiModel>> Enterprises()
            => (await CoralsaDbContext.Enterprises.ToListAsync()).Select(p => Node.Mapper.Map<EnterpriseApiModel>(p));
        ...
        public async Task<AuthResponse> Login(AuthRequest model)
        {
            ...
            var user = await CoralsaDbContext.Users.SingleOrDefaultAsync(p => p.Email.ToLowerInvariant() == model.I.ToLowerInvariant());
        ...
        {
            if (await CoralsaDbContext.Models.FindAsync(apiModel.Id) != null)
            ...
            model = CoralsaDbContext.Add(model).Entity;
            ...
            CoralsaDbContext.AddRange(rows);

            var enterprises = await CoralsaDbContext.Enterprises.ToListAsync();
        ...
        void GenerateRecords(List<Row> rows, List<Enterprise> enterprises, Template template)
        {
            ...
            CoralsaDbContext.AddRange(records);
        }

        public async Task ModelDelete(int id)
        {
            var model = CoralsaDbContext.Remove(new Model { Id = id }).Entity;
            await CoralsaDbContext.SaveChangesAsync().ConfigureAwait(false);
        }
        ...
        public async Task ModelRowsAdd(RowApiModel apiModel)
        {
            ...
            var model = await CoralsaDbContext.Models.Include(p => p.Templates).SingleAsync(p => p.Id == apiModel.ModelId);
            model.RowCount++;

            row.Id = model.RowCount;
            CoralsaDbContext.Rows.Add(row);
        ...
        {
            var model = await CoralsaDbContext.FindAsync<Model>(modelId);
            var row = CoralsaDbContext.Remove(new Row { Id = model.RowCount, ModelId = modelId }).Entity;

            model.RowCount--;
            var entity = CoralsaDbContext.Attach(model);
        ...
        public async Task<TemplateRecordsApiModel> TemplateRecords(TemplateApiModel apiModel)
        {
            ...
            var records = await (
                from record in CoralsaDbContext.Records
            ...
            var template = await CoralsaDbContext.Templates.FindAsync(apiModel.Year, apiModel.Month, apiModel.Day, apiModel.ModelId);
        ...
        public async Task TemplateLock(TemplateApiModel apiModel)
        {
            ...
            var entity = CoralsaDbContext.Attach(template);
        ...
        public async Task TemplateImport(TemplateApiModel apiModel)
        {
            ...
            var model = await CoralsaDbContext.Models.FindAsync(apiModel.ModelId);
            ...
            while ((line = stream.ReadLine()) != null)
            {
                ...
                var entity = CoralsaDbContext.Attach(record);
        ...
        {
            var model = await CoralsaDbContext.Models.FindAsync(apiModel.Model).ConfigureAwait(false);
            ...
            {
                var divisions = await CoralsaDbContext.Divisions.ToListAsync();
            ...
            {
                var groups = await CoralsaDbContext.Groups.Include(p => p.Division).Where(p => p.DivisionId == apiModel.Division).ToListAsync();
            ...
            {
                var enterprises = await CoralsaDbContext.Enterprises.Include(p => p.Group).Where(p => p.GroupId == apiModel.Group && p.DivisionId == apiModel.Division).ToListAsync();
        ...
        public async Task<IEnumerable<RecordColumnsApiModel>> ComputedRecords(RecordApiModel model)
        {
            ...
            //async Task<IEnumerable<RecordColumnsApiModel>> Compute() =>
                cache[model] = await (from record in CoralsaDbContext.Records.Include(p => p.Enterprise)
                       join template in CoralsaDbContext.Templates
