        ...
        sealed class Reflector
        {
            ...
            internal sealed class TypeData : IEquatable<TypeData>
            {
                ...
                public TypeData(Type type)
                {
                    ...
                    {
                        var shouldSerialize = ShouldSerializeField(Type, field);
            ...
            static bool ShouldSerializeField(Type type, FieldInfo field)
            {
                ...
                var property = type.GetAnyDeclaredProperty(propertyName);
...
#region Public
        ...
        /// <summary>
        /// Quick reset of the buffer content by setting the <see cref="length"/> and <see cref="Position"/> to 0.
...
#region ToConversions
        ...
        public Serializer ToSerializable()
        {
            ...
            return new Serializer { Buffer = buffer, length = buffer.Length, Capacity = buffer.Length };
        }
...
#region Object / Serializable
        ...
        void InternalSerializeObject<T>(T value, bool raw, IBackingSerializer serializer = null)
        {
            ...
            //    if (!raw)
            //    {
            //        for (var i = 0; i < SerializationList.Count; i++)
            //            if (SerializationList[i].Equals(value))
            //            {
            //                Serialize((i + 1) * (-1));
            //                return;
            //            }
            ...
            finally
            {
                if (UseObjectSerialization)
                {
                    ...
                    //if (SerializationStack.Count != 0)
                    //    SerializationStack.Peek().Next = null;
                    //else
                    if (SerializationStack.Count == 0)
                    {
                        if (CircularReferencesActive)
...
#region DeserializeString
        ...
        public string DeserializeString()
        {
            ...
            return Encoding.GetString(buffer, position - count, count);
        }
...
#region Object
        ...
        object InternalDeserializeObject<T>(object value, bool raw, IBackingSerializer serializer = null)
        {
            ...
            if (serializer != null)
                return serializer.Deserialize<T>(this, type: null, rawValue: raw);
            ...
            if (!raw)
            {
                ...
                //if (count < 0 && CircularReferencesActive)
                {
                    try { return CircularReferencesDictionary[count * (-1) - 1]; }
...
#region chars
        ...
        public char[] DeserializeChars() => AutoRaw ? ToCharArray() : DeserializeChars(0);

        public void DeserializeChars(char[] dest, int index, int count)
        {
#if !BRIDGE_NET
            unsafe
            {
                fixed (char* ptr = dest)
                    DeserializeChars(ptr + index, dest.Length, count);
            }
#endif
        }
...
#region bytes
        ...
        public byte[] DeserializeBytes() => AutoRaw ? ToByteArray() : DeserializeBytes(0);

        public byte[] DeserializeBytes(int count)
        {
            if (count < 0)
                throw new ArgumentOutOfRangeException("count", "The byte count to deserialize cannot be negative");
        ...
        public unsafe void DeserializeBytes(byte* dest, int destLength, int count)
        {
            if ((IntPtr)dest == IntPtr.Zero)
                throw new ArgumentNullException();
