    ...
    public abstract class Controller
    {
        ...
        protected virtual async Task<Result> OnActionExecutingAsync(ControllerAction action, params object[] parameters)
        {
            ...
            for (var i = 0; i < action.Filters.Count; i++)
            {
                ...
                {
                    if (!(result = await asyncActionFilter.OnActionExecutingAsync(actionExecutionContext).ConfigureAwait(false)))
                        return result;
                }
                else if (filter is IActionFilter actionFilter)
                    if (!(result = actionFilter.OnActionExecuting(actionExecutionContext)))
                        return result;
            }
        ...
        {
            //actionExecutionContext.Result = result;
            ...
            for (var i = action.Filters.Count - 1; i >= 0; i--)
            {
                ...
                if (filter is IAsyncActionFilter asyncActionFilter)
                {
                    if (!(result = await asyncActionFilter.OnActionExecutedAsync(actionExecutionContext).ConfigureAwait(false)))
                        return result;
                }
                else if (filter is IActionFilter actionFilter)
                    if (!(result = actionFilter.OnActionExecuted(actionExecutionContext)))
                        return result;
            }
            ...
            if ((parameters?.Length ?? 0) > 0)
                foreach (var parameter in parameters)
                    if (!(result = Utilities.Models.Validate(node, parameter)))
                    {
                        ...
                        //return result;
                        //return new Result(ResultCode.BadRequest, model: JsonConvert.SerializeObject(result.GetModel<IEnumerable<ValidationError>>(), Formatting.Indented));
                    }
        ...
        internal static async Task<Result> InvokeAsync(bool isCommand, Node node, Connection connection, ControllerAction action, params object[] parameters)
        {
            ...
            try
            {
                ...
                if (context.Result.HasValue)
                    value = CreateResultCopy(node, connection.Context.Result.Value, value, isCommand: isCommand);
            }
            ...
            //return await controller.OnActionExecutedAsync(action, parameters);
        }
