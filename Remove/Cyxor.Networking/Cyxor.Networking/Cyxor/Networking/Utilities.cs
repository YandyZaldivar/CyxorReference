        ...
        public static class Json
        {
            class JsonCommentSerializer
            {
                class JsonCommentWriter : JsonTextWriter
                {
                    ...
                    public JsonCommentWriter(StringWriter stringWriter, object target) : base(stringWriter)
                    {
                        Target = target;
                ...
                public virtual string Serialize(object target, bool excludeDefaultValues = false)
                {
                    ...
                    var jsonWriter = new JsonCommentWriter(new StringWriter(), target);

                    Serializer.Serialize(jsonWriter, target);
                    jsonWriter.ReorderLastComment();
                    return jsonWriter.StringWriter.GetStringBuilder().ToString();
                }
            ...
            public static string Serialize(object target, bool includeComments = false, bool excludeDefaultValues = false)
                => includeComments ? JsonCommentSerializer.Instance.Serialize(target) : JsonConvert.SerializeObject(target,
            ...
            public static object Deserialize(string value) => JsonCommentSerializer.Instance.Deserialize(value);
            public static object Deserialize<T>(string value) => JsonCommentSerializer.Instance.Deserialize<T>(value);
            public static object Deserialize(string value, Type type) => JsonCommentSerializer.Instance.Deserialize(value, type);
            public static void Populate(string value, object target) => JsonCommentSerializer.Instance.Populate(value, target);
        }
...
#pragma warning restore 0420
            ...
            {
                static AsyncAwaiterMethodBuilder<TResult> Default = new AsyncAwaiterMethodBuilder<TResult>();
