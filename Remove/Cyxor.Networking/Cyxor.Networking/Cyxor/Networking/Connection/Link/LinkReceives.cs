...
namespace Cyxor.Networking
{
    ...
    sealed partial class Link
    {
        internal sealed class LinkReceives : LinkProperty, IDisposable
        {
            ...
            Box TcpBox { get; set; }
            Serializer TcpBuffer { get; set; }
            SocketAsyncEventArgs TcpSaea { get; set; }

            Box UdpBox { get; set; }
            Serializer UdpBuffer { get; set; }
            SocketAsyncEventArgs UdpSaea { get; set; }
            ...
            internal void Initialize() => Countdown.Reset(initialCount: 1);

            internal bool Acquire(Packet packet, [System.Runtime.CompilerServices.CallerMemberName] string callerMemberName = null) => Acquire(Link.GetPacketSocket(packet), callerMemberName);

            internal bool Acquire(Socket socket, [System.Runtime.CompilerServices.CallerMemberName] string callerMemberName = null)
            {
                if (socket != null)
                {
            ...
            internal void Release(Packet packet, [System.Runtime.CompilerServices.CallerMemberName] string callerMemberName = null) => Release(Link.GetPacketSocket(packet), callerMemberName);

            internal void Release(Socket socket, [System.Runtime.CompilerServices.CallerMemberName] string callerMemberName = null)
            {
            ...
            void CheckBuffersAndBoxs22(Socket socket)
            {
                ...
                // CheckBuffersAndBoxs
                {
                    if (TcpBuffer.Length == 0 || PacketSize == 0)
                ...
                var bufferCount = serializer.Capacity - serializer.Length < Node.Config.MaxTCPReceiveBufferSize ?
                    serializer.Capacity - serializer.Length : Node.Config.MaxTCPReceiveBufferSize;

                var saea = socket == Link.TcpSocket ? TcpSaea : UdpSaea;

                //if (!Node.Config.Ssl.Enabled)
                    saea.SetBuffer(serializer.Buffer, serializer.Length, bufferCount);
                //else
            ...
            internal void SocketReceiveAsync(Socket socket)
            {
                ...
                var saea = socket == Link.TcpSocket ? TcpSaea : UdpSaea;
                saea.SetBuffer(buffer.Buffer, buffer.Length, buffer.Capacity - buffer.Length);
            ...
            void ParsePacket2(Socket socket, Serializer buffer)
            {
                Acquire(socket);

                try
                {
                    var frameRead = Frame.TryRead(Connection, buffer, out var frame);
            ...
            void ParsePacket(Socket socket, Serializer buffer)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        var progress = (int)((ulong)buffer.Length * 100 / (ulong)PacketSize);
