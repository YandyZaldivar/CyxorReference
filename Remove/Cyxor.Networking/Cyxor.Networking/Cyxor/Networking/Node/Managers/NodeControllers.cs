                    ...
                    try
                    {
                        if (e.Packet.IsCommand)
                        {
                            ...
                            if (e.Packet.IsHttpMessage)
                            {
                                ...
                                var contentLength = resultData.Length;
            ...
            static async Task<Result> ExecuteAsync(string cmdstr, Node node, Connection connection, Packet packet, int millisecondsTimeout, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    if (node.Context == null)
                        node.Context = new Context { JsonRequest = argument, Packet = packet };
                    ...
                    catch (Exception ex)
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                //        dictionary.Add(tuple[0], string.IsNullOrEmpty(tuple[1]) ? null : tuple[1]);
                                //}

                                //model = node.Mapper.Map(dictionary, dictionary.GetType(), command.RequestType);




                                //object Q22(string queryString, Type targetType)
                                //    => AgileObjects.AgileMapper.Mapper.Map(queryString.Split('&')
                                //        .Select(p => p.Split('=')).ToDictionary(p => p[0], p => p[1])).ToANew(targetType);

                                //var gy = Q22(argument, command.RequestType);

                                //model = Q22(argument, command.RequestType);




                                //model = node.Mapper.Map(argument.Split('&').Select(p => p.Split('='))
                                //    .ToDictionary(p => p[0], p => (object)p[1])).ToANew(command.RequestType);
