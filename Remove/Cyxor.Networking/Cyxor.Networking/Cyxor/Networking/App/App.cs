...
//#endif
            ...
            var cyxorLicenseLine = "  This program is free software; type 'license' for details  ";
        ...
        static async void CommandLoop()
        {
            ...
            try
            {
                //var acctionName = node.IsServer ? "running on" : "connected to";
                //var runningMessage = $"{node.CoreTypeName} {acctionName} {node.LocalEndPoint}";

                //node.Log(LogCategory.Title, indentLevel: -1, message: runningMessage);
                ...
                do
                {
                    ...
                    var line = (await Console.In.ReadLineAsync().ConfigureAwait(false)).Trim();
