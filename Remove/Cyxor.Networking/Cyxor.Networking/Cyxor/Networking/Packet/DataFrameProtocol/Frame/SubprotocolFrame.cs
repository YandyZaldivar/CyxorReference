...
namespace Cyxor.Networking
{
    ...
    struct SubprotocolFrame
    {
        ...
        public SubprotocolFrame(Node node)
        {
            Node = node;
            Connections = null;
        ...
        public void Serialize(Serializer serializer)
        {
            ...
            {
                serializer.EnsureCapacity(Payload.Length + 128);

                if (Compress)
                {
                    var compressStartPosition = serializer.Position;

                    var memoryStream = new MemoryStream(serializer.Buffer, serializer.Position, Payload.Length + 128);

                    using (var compressStream = new GZipStream(memoryStream, CompressionMode.Compress, leaveOpen: true))
                    {
                        compressStream.Write(Payload.Buffer, 0, Payload.Length);
                        compressStream.Flush();

                        if (memoryStream.Length - serializer.Position > Payload.Length)
                            Compress = false;
                        else
                        {
                            serializer.SetLength((int)memoryStream.Length - serializer.Position);
        ...
        public int TryReadSubProtocol(Connection connection, Serializer serializer)
        {
            ...
            if (bits1[ProtocolMap.CfgByte2])
            {
                ...
                Anonymous = bits2[ProtocolMap.Anonymous];
                Progress = bits2[ProtocolMap.Progress];
                Compress = bits2[ProtocolMap.Compress];
                Priority = (PacketPriority)bits2.Deserialize(ProtocolMap.Priority, 2);

                if (bits2[ProtocolMap.CfgByte3])
                    return FrameResult.Error;
            }

            Internal = bits1[ProtocolMap.Encrypted];
            Broadcast = bits1[ProtocolMap.Broadcast];

            if (bits1[ProtocolMap.Command])
            {
            ...
            QueryMode = (PacketQueryMode)bits1.Deserialize(ProtocolMap.QueryMode, 2);
            ...
            if (server != null)
                if (bits1[ProtocolMap.Sender])
                    return FrameResult.Error;

            if (!bits1[ProtocolMap.Address] && Broadcast)
            {
                if (server == null)
                    return FrameResult.Error;
            ...
            else if (bits1[ProtocolMap.Address])
            {
                ...
                if (server == null)
                {
                    if (!Broadcast || (Broadcast && (!Anonymous && !bits1[ProtocolMap.Sender])))
                        return FrameResult.Error;
