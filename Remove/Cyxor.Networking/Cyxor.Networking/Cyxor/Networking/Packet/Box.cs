    ...
    public sealed class Box : IBox//, ISerializable
    {
        ...
        */
        ...
        byte GetConfigByte()
        {
            ...
            bits[0] = Internal;
            bits[1] = IsCommand;
            bits.Serialize((int)QueryMode, ProtocolMap.QueryMode);
            bits[4] = Broadcast;
        ...
        byte GetConfigByte2()
        {
            ...
            bits[2] = Compress;
            bits[3] = Anonymous;
            bits.Serialize((int)Priority, ProtocolMap.Priority);
        ...
        internal static bool TryParseHeader(Serializer serializer, ref ReceiveProgress progress)
        {
            ...
            try
            {
                ...
                if (cfg[ProtocolMap.CfgByte2])
                {
                    var cfg2 = (BitSerializer)serializer.DeserializeByte();

                    progress.Tracking = cfg2[ProtocolMap.Progress];

                    if (cfg[ProtocolMap.CfgByte3])
                        throw new NotImplementedException();
                }

                if (cfg[ProtocolMap.Encrypted])
                    return false;

                progress.Type = serializer.DeserializeInt32();

                if (!cfg[ProtocolMap.Broadcast])
                    if (!cfg[ProtocolMap.Address])
        ...
        internal int TryLoad(Connection connection, Serializer serializer, bool pop = true)
        {
            ...
            if (cfg[ProtocolMap.CfgByte2])
            {
                ...
                priority = (PacketPriority)cfg2.Deserialize(ProtocolMap.Priority, 2);
                trackSendProgress = cfg2[ProtocolMap.Progress];
                anonymous = cfg2[ProtocolMap.Anonymous];
                compress = cfg2[ProtocolMap.Compress];
                error = cfg2[ProtocolMap.Error];

                if (cfg2[ProtocolMap.CfgByte3])
                    throw new NotImplementedException();
            }

            isCommand = cfg[ProtocolMap.Command];
            Internal = cfg[ProtocolMap.Encrypted];
            broadcast = cfg[ProtocolMap.Broadcast];
            queryMode = (PacketQueryMode)cfg.Deserialize(ProtocolMap.QueryMode, 2);
            ...
            if (server != null)
                if (cfg[ProtocolMap.Sender])
                    throw new Exception("Protocol Error.");

            if (!cfg[ProtocolMap.Address] && broadcast)
            {
                if (server == null)
                    throw new Exception("Protocol Error.");
            ...
            else if (cfg[ProtocolMap.Address])
            {
                ...
                if (server == null)
                {
                    if (!broadcast || (broadcast && (!anonymous && !cfg[ProtocolMap.Sender])))
                        throw new Exception("Protocol Error.");
            ...
            //if (cfg[ProtocolMap.Message])
            {
                ...
                if (size > serializer.Length - serializer.Position)
                    return serializer.Position + size;
                ...
                if (compress)
                {
                    ...
                    using (var inMs = new MemoryStream(serializer.Buffer, serializer.Position, size))
                ...
                else
                {
                    if (compress)
                        ...
                        message.SerializeRaw(serializer.Buffer, serializer.Position, size);

                    message.Position = 0;
                }

                serializer.Position = serializer.Position + size;
            }
            ...
            if (pop)
                if (serializer.Position == serializer.Length)
                    ...
                    serializer.Pop(serializer.Position);
        ...
        void Serialize()
        {
            ...
            if (!(message != null ? message.Length != 0 ? true : false : false))
            ...
            else
            {
                if (!compress)
                    ...
                    using (var gzip = new GZipStream(new MemoryStream(), CompressionMode.Compress))
                    {
                        ...
                        gzip.Write(message.Buffer, 0, message.Length);
                        gzip.Flush();

                        if (stream.Length < message.Length)
