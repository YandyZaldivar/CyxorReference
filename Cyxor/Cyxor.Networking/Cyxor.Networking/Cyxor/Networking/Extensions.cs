...
namespace Cyxor.Extensions
{
    ...
    public static class SerializerExtensions
    {
        ...
        public static void SaveToFile(this Serializer serializer, string filePath)
        {
            using (var stream = File.Open(filePath, FileMode.Create, FileAccess.Write, FileShare.None))
                ...
                {
                    stream.Write(serializer.Buffer, 0, serializer.Int32Length);
                    stream.Flush();
                }
        }

#if NET35 || NET40
        public static Task LoadFromFileAsync(this Serializer serializer, string filePath)
            => Task.Factory.StartNew(() => serializer.LoadFromFile(filePath));

        public static Task SaveToFileAsync(this Serializer serializer, string filePath)
            => Task.Factory.StartNew(() => serializer.SaveToFile(filePath));
#else
        ...
        public static async Task SaveToFileAsync(this Serializer serializer, string filePath)
        {
            using (var stream = File.Open(filePath, FileMode.Create, FileAccess.Write, FileShare.None))
                if (serializer.Buffer != null)
                {
                    await stream.WriteAsync(serializer.Buffer, 0, serializer.Int32Length).ConfigureAwait(false);
    ...
    public static class MiscExtensions
    {
        ...
        internal static void Dispose(this X509Certificate certificate) => certificate.Reset();
#endif
    }
}
/* { Cyxor } - .NET Core Backend Framework <http://www.cyxor.com/> */
