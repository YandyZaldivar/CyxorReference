...
namespace Cyxor.Networking.Config.Server
{
    ...
    public class DatabaseEngineConfig : ConfigProperty
    {
        ...
        public const string DefaultPort = "12484";
        ...
        public string Arguments
        {
            get
            {
                ...
                {
                    var index1 = arguments.IndexOf("--", i);

                    if (index1 > -1)
                    {
                        var index2 = arguments.IndexOf("--", index1 + 2);

                        if (index2 < 0)
                            index2 = arguments.Length;

                        items.Add(arguments.Substring(index1, index2 - index1).Trim());
                ...
                for (var i = 0; i < items.Count; i++)
                {
                    ...
                    if (pair[0].Replace("--", "").Trim().ToLowerInvariant() == "user")
                    {
                        userFound = true;
                        items[i] = pair[0] + '=' + User.ToString();
                    }
                    else if (pair[0].Replace("--", "").Trim().ToLowerInvariant() == "port")
                    {
                        portFound = true;
                        items[i] = pair[0] + '=' + Port.ToString();
                    }
                    else if (pair[0].Replace("--", "").Trim().ToLowerInvariant() == "pid-file")
                    {
                        pidfFound = true;
                        items[i] = pair[0] + '=' + RelativeProcessPidFile.ToString();
                    }
                }
                ...
                if (!pidfFound)
                    sb.Append($"--pid-file={RelativeProcessPidFile} ");
