...
namespace Cyxor.Controllers
{
    ...
    public abstract class MasterController : Controller
    {
        ...
        protected IQueryable<TModel> Query<TModel>(IQueryable<TModel> query, ReadApiModel readApiModel, int pageSize = -1)
            where TModel : class
        {
            //query = Include(query, readApiModel); // TODO: Review and redesign when ef core implements 'include filters'.
            query = Filter(query, readApiModel);
            query = Order(query, readApiModel);
            query = Paginate(query, readApiModel, pageSize);
